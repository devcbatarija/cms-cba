...
export default function PublicationTable() {
  ...
  const selectedPublications = useSelector((state) => state.publications.selectedPublications);
  ...
  const handleDelete = async () => {
    await axios.post("publication/delete/select", { ids: selectedPublications });
    setTimeout(() => {
      dispatch(getAllPublication());
  ...
  return (
    <div style={{ backgroundColor: "#f5f6fa", minHeight: "100vh", fontFamily: "'Segoe UI', 'Inter', sans-serif", color: "#1a1a2e" }}>
      ...
      <div style={{ padding: "28px 32px 20px", display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, margin: 0, color: CBA_NAVY }}>Publicaciones</h1>
      ...
      {selectedPublications.length > 0 && (
        <div style={{
          ...
          <span style={{ fontSize: 13, color: "#555", fontWeight: 500, fontFamily: "inherit" }}>
            {selectedPublications.length} seleccionada{selectedPublications.length !== 1 ? "s" : ""}
          </span>
          <button
      ...
      <div style={{ padding: "0 32px 20px" }}>
        <div style={{
          ...
          <div style={{ position: "relative", flex: "1 1 200px", maxWidth: 320 }}>
            <svg style={{ position: "absolute", left: 10, top: "50%", transform: "translateY(-50%)", color: "#aaa", pointerEvents: "none" }} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
      ...
      <div style={{ padding: "0 32px 40px" }}>
        {filtered.length === 0 ? (
          <div style={{
            ...
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#d0d3de" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: 18 }}>
            {filtered.map((pub, index) => {
              ...
              const isSelected = selectedPublications.includes(pub.id_Publicacion);
              const hasImage = pub.multimedia?.length > 0;

              return (
                <div
                  key={pub.id_Publicacion || index}
                  style={{
                    backgroundColor: "#fff",
                    borderRadius: 10,
                    overflow: "hidden",
                ...
                >
                  ...
                  <div style={{ position: "absolute", top: 10, left: 10, zIndex: 2 }}>
                    <input
                      type="checkbox"
                      checked={isSelected}
                      onChange={() => handleSelectPublication(pub.id_Publicacion)}
                  ...
                  <div style={{ position: "absolute", top: 10, right: 10, zIndex: 2 }}>
                    <span style={{
                      backgroundColor: pub.estado ? "rgba(26,122,56,0.88)" : "rgba(120,120,130,0.80)",
                      color: "#fff",
                      fontSize: 11,
                      fontWeight: 600,
                      padding: "3px 10px",
                      ...
                      {pub.estado ? "Visible" : "Oculto"}
                    </span>
                  </div>

                  {/* Imagen o placeholder */}
                  {hasImage ? (
                    <img src={pub.multimedia[0]} alt={pub.titulo} style={{ width: "100%", height: 175, objectFit: "cover", display: "block" }} />
                  ) : (
                  ...
                  <div style={{ padding: "14px 16px 10px", flex: 1, display: "flex", flexDirection: "column", gap: 7 }}>
                    ...
                    <div
                      ...
                      dangerouslySetInnerHTML={{ __html: pub.descripcion }}
                    />
                  </div>
                  ...
                  <div style={{ padding: "10px 16px 14px", borderTop: "1.5px solid #f0f2f7" }}>
                    <button
                      onClick={() => handleModal(pub.id_Publicacion)}
                      style={{
